-- Analytics seed: paid orders over the last 30 days
-- Run AFTER seed-demo.sql (requires menu items to exist)
...
DECLARE
  ...
  -- menu item ids (fetched by name so script is re-runnable)
...
BEGIN
  -- Use existing open shift (or null for historical records)
  SELECT id INTO shift_id FROM shifts WHERE restaurant_id = rid ORDER BY created_at DESC LIMIT 1;
  ...
  IF mi_cola IS NULL THEN
    RAISE EXCEPTION 'Menu items not found. Run seed-demo.sql first.';
  END IF;
  ...
  FOR day_offset IN REVERSE 29..0 LOOP
    ...
    IF EXTRACT(dow FROM (CURRENT_DATE - day_offset)) IN (0, 6) THEN
      num_orders := 7 + (day_offset % 3);  -- 7..9
    ELSE
      num_orders := 4 + (day_offset % 2);  -- 4..5
    END IF;

    FOR i IN 1..num_orders LOOP
      -- Spread orders across lunch (12-14h) and dinner (18-22h)
      ...
      -- Coffee after meal (every 5th order)
      IF i % 5 = 0 THEN
        INSERT INTO order_items (order_id, menu_item_id, name, quantity, price, status)
  ...
  RAISE NOTICE 'Analytics seed complete. Check Analytics dashboard.';
END $$;
