...
const ERROR_CODE_KEY: Record<string, string> = {
  ...
  WAREHOUSE_HAS_TABLES:  'warehouse_has_tables',
  WAREHOUSE_HAS_TRANSFERS: 'warehouse_has_transfers',
};

const DeleteWarehouseDialog: React.FC<Props> = ({ warehouse, onClose, onDeleted }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const msgKey = ERROR_CODE_KEY[code];
      if (msgKey) {
        setError(t(msgKey));
      } else {
        toast.error(t('error'));
      ...
      <div className="bg-surface border border-warm-line rounded-card shadow-card-lg w-full max-w-sm">
        <div className="p-6 space-y-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-sm font-black text-cream">{t('delete_warehouse')}</h3>
              <p className="text-sm text-cream-muted mt-1">
                {t('confirm_delete')} <span className="text-cream font-semibold">"{warehouse.name}"</span>?
              </p>
            </div>
          </div>
